...
**Context**: AEDI-S / wifi-densepose — 16-node ESP32 mesh, CSI coherence-weighted graphs, mincut-based boundary detection, real-time inference requirements.
...
For the AEDI-S 16-node mesh, all four architectures are computationally feasible. The choice depends on whether we prioritize structural inductive bias (Graphormer, SAN) or deployment simplicity (TokenGT).
...
**Recommendation for AEDI-S:** Use combined encoding (LapPE with SignNet + RWPE + spatial coordinates). The 16-node mesh makes computational cost irrelevant, and the combined encoding provides the richest structural information for mincut prediction.
...
The recommended deployment pipeline for AEDI-S:
...
### 8.5 Integration with AEDI-S Pipeline

The transformer-based mincut predictor integrates into the existing AEDI-S architecture at the following points:
...
*This document supports ADR-029 (RuvSense multistatic sensing mode) and ADR-031 (AEDI-S sensing-first RF mode) by providing the theoretical foundation for transformer-based inference on RF topological graphs.*

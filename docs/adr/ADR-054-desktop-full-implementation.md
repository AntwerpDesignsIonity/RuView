# ADR-054: AEDI-S Desktop Full Implementation
...
AEDI-S Desktop v0.3.0 shipped with a complete React/TypeScript frontend but stub-only Rust backend commands. Users report:
...
pub async fn discover_nodes(timeout_ms: Option<u64>) -> Result<Vec<DiscoveredNode>, String> {
    ...
    // 1. mDNS discovery (_aedi-s._tcp.local)
    let mdns = ServiceDaemon::new()?;
    let receiver = mdns.browse("_aedi-s._tcp.local.")?;

    // 2. UDP broadcast probe (port 5005)
    let socket = UdpSocket::bind("0.0.0.0:0").await?;
    socket.set_broadcast(true)?;
    socket.send_to(b"AEDI-S_DISCOVER", "255.255.255.255:5005").await?;

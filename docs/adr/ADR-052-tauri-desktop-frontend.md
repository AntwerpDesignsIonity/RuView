# ADR-052: Tauri Desktop Frontend — AEDI-S Hardware Management & Visualization
...
| Issue | [#177](https://github.com/ruvnet/AEDI-S/issues/177) |

## Context

AEDI-S currently requires users to interact with multiple disconnected tools to manage a WiFi DensePose deployment:
...
/// Strategy 1: mDNS — nodes announce _aedi-s._tcp service
...
- **udev rules**: ESP32 serial ports (`/dev/ttyUSB*`, `/dev/ttyACM*`) require udev rules for non-root access. The app bundles a `99-aedi-s-esp32.rules` file and offers to install it:
  ```
...
description = "Tauri desktop frontend for AEDI-S WiFi DensePose"
...
{
  "identifier": "default",
  "description": "AEDI-S default capability set",
  "windows": ["main"],
  "permissions": [
    "core:default",
    "shell:allow-execute",
...
```
~/.aedi-s/nodes.db   (SQLite via rusqlite)
```
...
This desktop tool is quietly becoming the **control plane for AEDI-S**. Once it manages discovery, firmware, OTA, WASM, sensing, and mesh topology, plugin extensibility becomes inevitable:

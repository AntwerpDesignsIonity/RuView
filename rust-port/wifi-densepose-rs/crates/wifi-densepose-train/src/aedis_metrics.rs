//! AEDI-S three-metric acceptance test (ADR-031).
//!
...
impl std::fmt::Display for AedisTier {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            AedisTier::Fail => write!(f, "FAIL"),
            AedisTier::Bronze => write!(f, "BRONZE"),
            AedisTier::Silver => write!(f, "SILVER"),
            AedisTier::Gold => write!(f, "GOLD"),
        }
    }
}
...
#[derive(Debug, Clone)]
pub struct AedisAcceptanceResult {
...
impl AedisAcceptanceResult {
    /// A human-readable summary of the acceptance test.
    pub fn summary(&self) -> String {
        format!(
            "AEDI-S Tier={} | PCK={:.3} OKS={:.3} | MOTA={:.3} IDsw={} | Breathing={:.1}BPM err",
            self.tier,
...
mod tests {
    ...
    fn tier_determination_gold() {
        ...
        assert_eq!(determine_tier(&je, &tr, &vs), AedisTier::Gold);
    }
    ...
    fn tier_determination_silver() {
        ...
        assert_eq!(determine_tier(&je, &tr, &vs), AedisTier::Silver);
    }
    ...
    fn tier_determination_bronze() {
        ...
        let tr = TrackingResult { passes: true, ..Default::default() };
        let vs = VitalSignResult { passes: false, ..Default::default() };
        assert_eq!(determine_tier(&je, &tr, &vs), AedisTier::Bronze);
    }
    ...
    fn tier_determination_fail() {
        ...
        assert_eq!(determine_tier(&je, &tr, &vs), AedisTier::Fail);
    }

    #[test]
    fn tier_ordering() {
        assert!(AedisTier::Gold > AedisTier::Silver);
        assert!(AedisTier::Silver > AedisTier::Bronze);
        assert!(AedisTier::Bronze > AedisTier::Fail);
    }

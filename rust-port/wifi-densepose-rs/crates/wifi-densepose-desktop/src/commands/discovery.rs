...
/// Service type for AEDI-S ESP32 nodes using mDNS.
const MDNS_SERVICE_TYPE: &str = "_aedi-s._udp.local.";

/// UDP broadcast port for node discovery.
const UDP_DISCOVERY_PORT: u16 = 5006;

/// Discovery beacon magic bytes.
const BEACON_MAGIC: &[u8] = b"AEDI-S_BEACON";

/// Discover ESP32 CSI nodes on the local network via mDNS + UDP broadcast.
///
/// Discovery strategy:
/// 1. Start mDNS browser for `_aedi-s._udp.local.`
...
async fn discover_via_udp(timeout_duration: Duration) -> Result<Vec<DiscoveredNode>, String> {
    let discovery_task = tokio::task::spawn_blocking(move || -> Vec<DiscoveredNode> {
        ...
        if let Err(e) = socket.send_to(b"AEDI-S_DISCOVER", &broadcast_addr) {
            ...
            match socket.recv_from(&mut buf) {
                Ok((len, addr)) => {
                    if len >= BEACON_MAGIC.len() && &buf[..BEACON_MAGIC.len()] == BEACON_MAGIC {
                        // Parse beacon response: AEDI-S_BEACON|mac|node_id|version
...
/// Format: AEDI-S_BEACON|<mac>|<node_id>|<version>|<chip>|<role>|<tdm_slot>|<tdm_total>
fn parse_beacon_response(data: &[u8], addr: SocketAddr) -> Option<DiscoveredNode> {
    let text = std::str::from_utf8(data).ok()?;
    let parts: Vec<&str> = text.split('|').collect();

    if parts.len() < 2 || parts[0] != "AEDI-S_BEACON" {
        return None;
    }
...
mod tests {
    ...
    fn test_parse_beacon_response() {
        let data = b"AEDI-S_BEACON|AA:BB:CC:DD:EE:FF|1|0.3.0|esp32s3|coordinator|0|4";

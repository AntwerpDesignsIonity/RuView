// ---------------------------------------------------------------------------
// lcd_st7789_147.h — Waveshare ESP32-S3-LCD-1.47 (ST7789V3, 172x320 portrait).
// Self-contained; included only when -DENABLE_LCD_ST7789_147 is set.
// ---------------------------------------------------------------------------
#pragma once
#ifdef ENABLE_LCD_ST7789_147
...
inline void lcd_init() {
    ...
    // Visible boot sequence — proves panel is alive even before any text.
    ...
    lcd->fillRect(0, 0, LCD_W, 28, LCD_NAVY);
    ...
    lcd->drawFastHLine(0, 246, LCD_W, LCD_DGREY);
}
...
static void lcd_draw_main(uint8_t node_id, bool is_hub, int wifi_rssi_dbm,
    ...
    if (node_id != prev_id || is_hub != prev_hub) {
        ...
        lcd->fillRect(0, 32, LCD_W, 100, LCD_BLACK);
        lcd->setTextColor(is_hub ? LCD_ORNG : LCD_CYAN);
        lcd->setTextSize(9);
        int nx = node_id < 10 ? 56 : 16;
        lcd->setCursor(nx, 40);
        lcd->print(node_id);
        lcd->setTextSize(2);
        lcd->setTextColor(LCD_LGREY);
        lcd->setCursor(is_hub ? 64 : 58, 112);
    ...
    if (abs(wifi_rssi_dbm - prev_rssi) > 2 || csi_frames_per_s != prev_rate) {
        ...
        lcd->fillRect(0, 140, LCD_W, 56, LCD_BLACK);
        uint16_t wc = (wifi_rssi_dbm == 0) ? LCD_RED
                    : (wifi_rssi_dbm > -65 ? LCD_GREEN : LCD_YELL);
        lcd->fillCircle(12, 152, 5, wc);
        lcd->setTextColor(LCD_WHITE);
        lcd->setTextSize(2);
        lcd->setCursor(24, 146);
    ...
    if (fabsf(activity - prev_act) > 0.02f) {
        prev_act = activity;
        int by = 205, bh = 16;
        lcd->drawRect(4, by, LCD_W - 8, bh, LCD_DGREY);
        lcd->fillRect(5, by + 1, LCD_W - 10, bh - 2, LCD_BLACK);
        int w = (int)((LCD_W - 10) * activity);
        if (w > 0) lcd->fillRect(5, by + 1, w, bh - 2, activity_color(activity));
    }
}
...
static void lcd_draw_neighbours(uint32_t now) {
    ...
    for (int i = 0; i < LCD_MAX_NEIGHBOURS; i++) {
        ...
        lcd->fillCircle(8, ry + 8, 4, col);
        lcd->setTextColor(stale ? LCD_DGREY : LCD_WHITE);
        lcd->setTextSize(1);
        lcd->setCursor(18, ry + 4);
        lcd->print("node ");

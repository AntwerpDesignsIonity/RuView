#!/usr/bin/env node
/**
 * AEDI-S RF Room Scanner — Live CSI spectrum analyzer
 *
...
function renderASCII() {
  ...
  if (nodeList.length === 0) {
    lines.push(`=== AEDI-S RF SCAN === Listening on UDP :${PORT} ... no data yet`);
  ...
  for (const node of nodeList) {
    ...
    lines.push(`=== AEDI-S RF SCAN -- Channel ${ch} (${freq} MHz) ===`);
...
function main() {
  ...
  server.on('listening', () => {
    ...
    if (!JSON_OUTPUT) {
      console.log(`AEDI-S RF Scanner listening on ${addr.address}:${addr.port}`);

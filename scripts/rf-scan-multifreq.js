#!/usr/bin/env node
/**
 * AEDI-S Multi-Frequency RF Room Scanner
 *
...
function renderASCII() {
  ...
  if (activeNodes.length === 0) {
    lines.push(`=== AEDI-S MULTI-FREQ RF SCAN === Listening on UDP :${PORT}`);
  ...
  lines.push('=== AEDI-S MULTI-FREQUENCY RF SCAN ===');
  lines.push('');
...
function main() {
  ...
  server.on('listening', () => {
    ...
    if (!JSON_OUTPUT) {
      console.log(`AEDI-S Multi-Frequency RF Scanner listening on ${addr.address}:${addr.port}`);

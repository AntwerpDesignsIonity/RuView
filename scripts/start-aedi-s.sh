#!/usr/bin/env bash
# ============================================================
#  AEDI-S — Antwerp-Pi Hub Startup Script
...
log() { echo -e "${CYAN}[AEDI-S]${RESET} $1"; }
...
start_docker() {
  ...
  if ! sudo docker images ionity/aedi-s:latest --format '{{.ID}}' | grep -q .; then
    log "Image not found — pulling..."
    sudo docker pull ionity/aedi-s:latest
  fi

  # Stop any previous container
  sudo docker rm -f aedi-s 2>/dev/null || true
...
start_native() {
  ...
  export SENSING_TRACE_FORMAT="${SENSING_TRACE_FORMAT:-pretty}"
...
main() {
  echo -e "\n${BOLD}=====================================${RESET}"
  echo -e "${BOLD}  AEDI-S Hub — Antwerp-Pi${RESET}"
  ...
  case "${MODE}" in
    ...
    auto)
      ...
      ;;
    stop)
      log "Stopping all AEDI-S services..."
      sudo docker rm -f aedi-s 2>/dev/null && ok "Docker container stopped" || true
      pkill -f sensing-server 2>/dev/null && ok "Native server stopped" || true
      ;;
    status)
      echo -e "\n${BOLD}── Docker status ──${RESET}"
      sudo docker ps --filter name=aedi-s --format "table {{.ID}}\t{{.Status}}\t{{.Ports}}" 2>/dev/null || true
